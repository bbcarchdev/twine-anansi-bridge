...
static int ingest_info(AWSS3BUCKET *bucket, const char *resource, json_t **dict);
...
static int ingest_headers(json_t *dict, const char *graph, librdf_model *model);
...
static int
ingest_info(AWSS3BUCKET *bucket, const char *resource, json_t **dict)
{
	AWSREQUEST *req;
	CURL *ch;
...
static int
ingest_headers(json_t *dict, const char *graph, librdf_model *model)
{
	json_t *values, *value;
	const char *name;
	size_t index;
