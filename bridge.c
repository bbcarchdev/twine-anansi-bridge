...
static const char *short_progname = TWINE_APP_NAME;

int
main(int argc, char **argv)
{
	const char *dburi, *mquri, *bucket, *t;
	SQL *sql;
	MQ *messenger;
	...
	{
		/* Obtain the message queue connection URI */
		mquri = config_getptr_unlocked(TWINE_APP_NAME ":mq", NULL);
		if(!mquri)
		{
			/* If that fails, try the global 'uri' key in the [mq] section */
			mquri = config_getptr_unlocked("mq:uri", NULL);
		}
		if(!mquri)
		{
			log_printf(LOG_CRIT, TWINE_APP_NAME ": failed to determine message queue URI\n");
			return 1;
		}
		log_printf(LOG_DEBUG, TWINE_APP_NAME ": establishing connection to <%s>\n", mquri);
		messenger = mq_connect_send(mquri, NULL, NULL);
		if(!messenger)
		{
			log_printf(LOG_CRIT, TWINE_APP_NAME ": failed to create message queue client for <%s>\n", mquri);
			return 1;
		}
		if(mq_error(messenger))
		{
			log_printf(LOG_CRIT, TWINE_APP_NAME ": failed to establish message queue receiver: %s\n", mq_errmsg(messenger));
...
config_defaults(void)
{
	...
	config_set_default("log:ident", short_progname);
